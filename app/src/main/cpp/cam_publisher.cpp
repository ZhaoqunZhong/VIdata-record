...
PerfMonitor fps_monitor;
int CamPublisher::fps_;
...
/*    int32_t num_planes;
    ...
    rotate(rgb, rgbMsg.rgbMat, cv::ROTATE_90_CLOCKWISE);
#endif

    rgbCallback(rgbMsg);
    AImage_delete(image);
    fps_monitor.update(CamPublisher::fps_);

/*    std::thread t(updateCamPreview, rgbMsg.yMat);
    t.detach();*/
}
...
void rgb_onCaptureCompleted(
    ...
    // LOGI("rgb ACAMERA_SENSOR_ROLLING_SHUTTER_SKEW %ld", rgb_readout_time);

    ACameraMetadata_getConstEntry(result, ACAMERA_SENSOR_EXPOSURE_TIME, &entry);
//    int64_t exposure_time = entry.data.i64[0];
//    LOGI("rgb ACAMERA_SENSOR_EXPOSURE_TIME %ld", exposure_time);
    rgb_realtime_exposure = entry.data.i64[0];
...
//    rgb_readout_time = static_cast<int>(fs["rgb_readout_time"]);
...
//    pthread_create(&depth_capture_t, nullptr, depthManualThreadWrapper, this);
    ...
    // ANativeWindow_setBuffersGeometry(preview_native_window, rgb_height_, rgb_width_,
